...
from shoal.settings import get_current_selected_game, set_current_selected_game
    ...
    def select_changed(self, event: Select.Changed) -> None:
        ...
        set_current_selected_game(
            data_structures.get_enum_from_val(
                data_structures.Games, self.options[event.value][0]
            )
        )
        ...
        print_to_log_window(
            f'Loaded settings for "{get_current_selected_game().value}"'
        )
