...
def get_current_selected_game() -> data_structures.Games:
    ...
    last_selected_game = global_settings.get(
        "last_selected_game", "Call of Duty World at War"
    )
    last_selected_game = data_structures.get_enum_from_val(
        data_structures.Games, last_selected_game
    )
...
def t7x_check(username: str):
    from shoal.game_clients.game_clients import (
        get_current_config_path_for_alterware_games,
    )
...
def remove_username(username: str):
    ...
    if len(usernames) <= 1:
        ...
        print_to_log_window(
            "Username removal failed, you cannot remove a username when you only have one username"
        )
        return
...
def set_currently_selected_game_mode(game_mode: data_structures.GameModes):
    ...
    SETTINGS["games"][get_current_selected_game().value]["last_selected_game_mode"] = (
        game_mode.value
    )
...
def set_global_args(args: list[str]):
    SETTINGS.setdefault("global", {})["global_args"] = args
    save_settings()
...
def set_game_directory(game_directory: str):
    if not os.path.isdir(game_directory):
        error_message = (
            f'The following directory you choose does not exist: "{game_directory}"'
        )
...
def remove_game_specific_arg(game_arg: str):
    ...
    if (
        ...
        == data_structures.Games.CALL_OF_DUTY_MODERN_WARFARE_III_2011
    ):
        if get_currently_selected_game_mode() == data_structures.GameModes.MULTIPLAYER:
            remove_line_from_config(
                get_plutonium_modern_warfare_iii_config_path(), game_arg
            )
    if get_current_selected_game() == data_structures.Games.CALL_OF_DUTY_BLACK_OPS_III:
...
def get_alterware_launcher_path() -> str:
    ...
    alterware_launcher_path = os.path.normpath(
        f"{SCRIPT_DIR}/assets/alterware_launcher/alterware-launcher.exe"
    )
    ...
    if not os.path.isfile(alterware_launcher_path):
        ...
        print_to_log_window(
            "Downloading the alterware launcher to the following location"
        )
