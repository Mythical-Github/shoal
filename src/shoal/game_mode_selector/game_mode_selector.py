...
class GameModeSelector(Static):
    def compose(self) -> ComposeResult:
        ...
        self.my_select: Select[int] = Select(
            self.options, allow_blank=False, value=main_value
        )
    ...
    def select_changed(self, event: Select.Changed) -> None:
        if get_currently_selected_game_mode() != data_structures.get_enum_from_val(
            data_structures.GameModes, self.options[event.value][0]
        ):
            set_currently_selected_game_mode(
            ...
            print_to_log_window(
                f'Changed selected game mode for "{get_current_selected_game().value}" to "{get_currently_selected_game_mode().value}"'
            )
