...
def get_plutonium_appdata_dir() -> str:
    if get_use_staging():
        pluto_appdata_path = os.path.join(
            os.environ["LOCALAPPDATA"], "Plutonium-staging"
        )
    else:
...
def get_plutonium_bootstrapper() -> str:
    return os.path.normpath(
        f"{get_plutonium_appdata_dir()}/bin/plutonium-bootstrapper-win32.exe"
    )
...
def get_plutonium_modern_warfare_iii_config_path() -> str:
    return os.path.normpath(
        f"{get_plutonium_appdata_dir()}/{get_plutonium_modern_warfare_iii_config_path_suffix()}"
    )
...
def install_plutonium_normal_branch():
    ...
    args = [f'--directory "{get_plutonium_appdata_dir()}"', "--launcher"]
    for arg in args:
...
def get_plutonium_updater_path() -> str:
    return os.path.normpath(
        f"{SCRIPT_DIR}/assets/plutonium_updater/plutonium-updater.exe"
    )
...
def download_plutonium_updater():
    ...
    plutonium_updater_zip = os.path.normpath(
        f"{os.path.dirname(plutonium_updater_path)}/plutonium_updater.zip"
    )
    download_file(get_plutonium_updater_url(), plutonium_updater_zip)
    from shoal.logger import print_to_log_window

    print_to_log_window(
        'Plutonium Updater Repo Link: "https://github.com/mxve/plutonium-updater.rs"'
    )
    print_to_log_window(
        'Plutonium Updater License: "https://github.com/mxve/plutonium-updater.rs/blob/master/LICENSE"'
    )
