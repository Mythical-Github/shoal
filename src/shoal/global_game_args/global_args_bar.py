...
from shoal.settings import get_current_selected_game, get_global_args
...
class AddGlobalArgButton(Static):
    ...
    def on_button_pressed(self) -> None:
        ...
        app.push_screen(
            global_args_screen.GlobalArgsScreen(widget_to_refresh=self.parent.parent)
        )
...
class RemoveGlobalArgButton(Static):
    ...
    def on_button_pressed(self) -> None:
        ...
        print_to_log_window(
            f'Attempting to remove the following global argument: "{global_arg}"'
        )

        remove_global_arg(global_arg)
        if (
            get_current_selected_game().value
            == data_structures.Games.CALL_OF_DUTY_MODERN_WARFARE_III_2011.value
        ):
            remove_line_from_config(
                get_plutonium_modern_warfare_iii_config_path(), global_arg
            )
...
class GlobalArgsSection(Static):
    def compose(self) -> ComposeResult:
        ...
        self.combo_box: Select[int] = Select(
            self.options, allow_blank=allow_global_args_blank(), prompt="None"
        )
